...
**OLED rendering has two ownership modes — pick the right path.** The 128×64 display (`io/oled_display.c`, driver `drivers/display_ug2864hsweg01.c`) is owned either by the firmware (`OLED_MODE_SYSTEM`, telemetry) or by user code (`OLED_MODE_USER`, set via `Oled_Mode(User)` in `onLoopStart`). The public pixel API (`Oled_Clear`/`Oled_Text`/`Oled_Update` in `API/Oled.h`) draws into a framebuffer that `Oled_Update` pushes **only in User mode** — it is a no-op at boot/System mode. System-mode text uses the 8 px character grid (`i2c_OLED_set_xy`/`i2c_OLED_send_string`, 21×8 cells, rows 1–6 usable). Two traps proven in practice: (1) `Oled_Print()` is declared with C++ linkage in `API/Oled.h` but **defined in C** (`oled_display.c`), so it fails to link from any `.cpp` — call the grid primitives or push a framebuffer directly instead. (2) The boot splash is `plutoStartUpPage()` in `PlutoPilot.cpp` (it replaces the stock `OledStartUpPage()` at the top of `loop()`, ~`mw.cpp:1117`); it must **redraw every frame** and toggle the shared `OledStartupPageEnd` flag, because the `devmode` false→true transition in `OledDisplaySystemData` triggers a one-time full clear that a single (diff-cached) push would lose.
